...
def playMatches(
    player1, player2, EPISODES, logger, turns_until_tau0, memory=None, goes_first=0
):
            ...
            for r in range(env.grid_shape[0]):
                logger.info(
                    [
                        ...
                        for x in pi[
                            env.grid_shape[1]
                            * r : (env.grid_shape[1] * r + env.grid_shape[1])
                        ]
                    ]
                )
            logger.info(
                "MCTS perceived value for %s: %f",
                state.pieces[str(state.playerTurn)],
                np.round(MCTS_value, 2),
            )
            logger.info(
                "NN perceived value for %s: %f",
                state.pieces[str(state.playerTurn)],
                np.round(NN_value, 2),
            )
            logger.info("====================")
