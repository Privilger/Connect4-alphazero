...
def main():
    ...
    if initialise.INITIAL_MEMORY_VERSION == None:
        ...
        print(
            "LOADING MEMORY VERSION " + str(initialise.INITIAL_MEMORY_VERSION) + "..."
        )
        memory = pickle.load(
            open(
                run_archive_folder
    ...
    plot_model(
        current_NN.model, to_file=run_folder + "models/model.png", show_shapes=True
    )
        ...
        if len(memory.ltmemory) >= config.MEMORY_SIZE:
            ...
            if iteration % 5 == 0:
                pickle.dump(
                    memory,
                    open(
                        run_folder + "memory/memory" + str(iteration).zfill(4) + ".p",
                        "wb",
                    ),
                )
            ...
            for s in memory_samp:
                ...
                lg.logger_memory.info(
                    "MCTS VALUE FOR %s: %f", s["playerTurn"], s["value"]
                )
                lg.logger_memory.info(
                ...
                lg.logger_memory.info(
                    "THE MCTS ACTION VALUES: %s", ["%.2f" % elem for elem in s["AV"]]
                )
                lg.logger_memory.info(
                ...
                lg.logger_memory.info(
                    "BES PRED ACTION VALUES: %s", ["%.2f" % elem for elem in best_probs]
                )
            ...
            print(f"MEMORY SIZE: {len(memory.ltmemory)}")
